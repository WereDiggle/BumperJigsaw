...
uniform float eps_inc = 1.01;
uniform float eps_dec = 0.99;
...
void fragment() {
	...
	if (UV[0] < margin_size*eps_dec) {
		color.a = 0.0;
	}
	if (UV[0] > (1.0 - margin_size)*eps_inc) {
		color.a = 0.0;
	}
	if (UV[1] < margin_size*eps_dec) {
		color.a = 0.0;
	}
	if (UV[1] > (1.0 - margin_size)*eps_inc) {
		color.a = 0.0;
	}
	...
	if (tab_facing[0] >= 1.0) {
		if (length(left - left_size - UV) < tab_size[0]*eps_inc) {
			color.a = 1.0;
		}
	} 
	if (tab_facing[0] <= -1.0) {
		if (length(left + left_size - UV) < tab_size[0]*eps_dec) {
			color.a = 0.0;
		}
	}
	...
	if (tab_facing[1] >= 1.0) {
		if (length(top - top_size - UV) < tab_size[1]*eps_inc) {
			color.a = 1.0;
		}
	} 
	if (tab_facing[1] <= -1.0) {
		if (length(top + top_size - UV) < tab_size[1]*eps_dec) {
			color.a = 0.0;
		}
	}
	...
	if (tab_facing[2] >= 1.0) {
		if (length(right + right_size - UV) < tab_size[2]*eps_inc) {
			color.a = 1.0;
		}
	}
	if (tab_facing[2] <= -1.0) {
		if (length(right - right_size - UV) < tab_size[2]*eps_dec) {
			color.a = 0.0;
		}
	}
	...
	if (tab_facing[3] >= 1.0) {
		if (length(bot + bot_size - UV) < tab_size[3]*eps_inc) {
			color.a = 1.0;
		}
	}
	if (tab_facing[3] <= -1.0) {
		if (length(bot - bot_size - UV) < tab_size[3]*eps_dec) {
			color.a = 0.0;
		}
	}
